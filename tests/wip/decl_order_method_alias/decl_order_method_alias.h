...
public:
  ...
  std::optional<rfile> write_r(rfile x0_, uint64_t x1_) const {
    return RegFile::template replace_nth<Rv>(x0_, x1_, *this);
  }
};
...
const std::optional<rfile> written =
    Rv::rs(UINT64_C(7))
        .write_r(List<Rv>::cons(
                     Rv::ru(),

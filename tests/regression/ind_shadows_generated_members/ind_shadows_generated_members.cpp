...
uint64_t IndShadowsGeneratedMembers::depth(
    const IndShadowsGeneratedMembers::variant_t0 &x) {
  if (std::holds_alternative<
          typename IndShadowsGeneratedMembers::variant_t0::V_mut>(x.v())) {
    const auto &[a0] =
        std::get<typename IndShadowsGeneratedMembers::variant_t0::V_mut>(x.v());
    return a0;
  } else {
    const auto &[a0] =
        std::get<typename IndShadowsGeneratedMembers::variant_t0::V_>(x.v());
    return (depth(*a0) + 1);
  }
}
